...
#Function to swap two animated bars
...
def _insertion_sort():
    ...
    for i in range(len(lengthList)):
        ...
        while pos > 0 and lengthList[pos - 1] > cursor:
            ...
            yield             #generator used to make memory management much efficient and runtime is faster with it.                         
            pos -= 1                                   
...
#Calling th e defined fuctions with animatw() method defined:
def insertion_sort():     
    global worker
...
def generate():
    ...
    #Creating a rectangle and number od bars are defined here:
    for bar in range(1, 151):
        randomY = random.randint(1, 360)     #TKinter() ka canvas method used here
        bar = canvas.create_rectangle(barstart, randomY, barend, 365, fill='silver')
        barList.append(bar)
        barstart += 10
        barend += 10
    ...
    #Maximum is color Red
    #Minimum is color Black
    for i in range(len(lengthList)-1):
...
window.geometry('1520x450')

canvas = tk.Canvas(window, width='1510', height='400')
